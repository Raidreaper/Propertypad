...
class UserController {
    ...
    async updateProfile(req, res) {
        try {
            ...
            const user = await User_1.default.findByIdAndUpdate(req.user.id, { firstName, lastName, email, phone, company, address }, { new: true, runValidators: true }).select('-password');
            if (!user) {
                return res.status(404).json({ message: 'User not found' });
            }
            res.json(user);
    ...
    async updateSettings(req, res) {
        try {
            ...
            const user = await User_1.default.findByIdAndUpdate(req.user.id, { settings: { notifications, theme, language, timezone } }, { new: true, runValidators: true }).select('-password');
            if (!user) {
                return res.status(404).json({ message: 'User not found' });
            }
            res.json(user);
